...
class Category(Base):
    ...
    category_id = Column(
        Integer, primary_key=True, autoincrement=True
    )  # แก้ไข: เปลี่ยน 'id' เป็น 'category_id'
...
class Attraction(Base):
    ...
    id = Column(
        Integer, primary_key=True, autoincrement=True
    )  # รักษานามสกุล 'id' เนื่องจาก schema แสดง 'attractions_id_seq'
    ...
    )  # อัปเดต FK ให้ชี้ไปที่ 'Category.category_id'
...
class Image(Base):
    ...
    id = Column(
        Integer, primary_key=True, autoincrement=True
    )  # รักษานามสกุล 'id' เนื่องจาก schema แสดง 'Image_image_id_seq'
    attraction_id = Column(
        Integer, ForeignKey("attractions.id"), nullable=False
    )  # ชื่อตาราง "attractions" ตรงแล้ว
...
class Review(Base):
    ...
    id = Column(
        Integer, primary_key=True, autoincrement=True
    )  # รักษานามสกุล 'id' เนื่องจาก schema แสดง 'Review_review_id_seq'
    attraction_id = Column(
        Integer, ForeignKey("attractions.id"), nullable=False
    )  # ชื่อตาราง "attractions" ตรงแล้ว
    user_id = Column(
        Integer, ForeignKey("User.user_id"), nullable=False
    )  # อัปเดต FK ให้ชี้ไปที่ 'User.user_id'
...
class Favorite(Base):
    ...
    id = Column(
        Integer, primary_key=True, autoincrement=True
    )  # รักษานามสกุล 'id' เนื่องจาก schema แสดง 'Favorite_favorite_id_seq'
    user_id = Column(
        Integer, ForeignKey("User.user_id"), nullable=False
    )  # อัปเดต FK ให้ชี้ไปที่ 'User.user_id'
    attraction_id = Column(
...
class AttractionTag(Base):
    ...
    attraction_id = Column(
        Integer, ForeignKey("attractions.id"), primary_key=True
    )  # ชื่อตาราง "attractions" ตรงแล้ว
    tag_id = Column(
        Integer, ForeignKey("Tag.tag_id"), primary_key=True
    )  # อัปเดต FK ให้ชี้ไปที่ 'Tag.tag_id'

    __table_args__ = (
        UniqueConstraint("attraction_id", "tag_id", name="_attraction_tag_uc"),
    )  # ป้องกันแท็กซ้ำสำหรับสถานที่เดียวกัน
...
def save_users_to_db(users_data):
    ...
    try:
        ...
        session.commit()
        print(
            f"บันทึกผู้ใช้สำเร็จ! บันทึกไป {saved_count} รายการ, ข้ามไป {skipped_count} รายการ"
        )
...
def save_attractions_and_related_data(
    attractions_data, all_user_ids, all_category_ids, all_tag_ids
):
    ...
    try:
        for attr_item in attractions_data:
            ...
            if not category_id:
                print(
                    f"ข้ามสถานที่ '{name}' เนื่องจากไม่พบหมวดหมู่ '{attr_item.get('category_name')}'"
                )
                skipped_attractions_count += 1
                continue
            ...
            if all_tag_ids:
                ...
                for tag_name in selected_tag_names:
                    ...
                    attraction_tag = AttractionTag(
                        attraction_id=attraction.id, tag_id=tag_id
                    )
            ...
            if all_user_ids and random.random() < 0.3:  # 30% ที่จะมีคนกด Favorite
                ...
                try:
                    favorite = Favorite(
                        user_id=random_user_id, attraction_id=attraction.id
                    )
                    session.add(favorite)
                except Exception as e:
                    # อาจเกิด UniqueConstraint error ถ้าผู้ใช้คนเดิม favorite สถานที่เดิมซ้ำ
                    print(
                        f"ข้อผิดพลาดในการบันทึก Favorite สำหรับ {attraction.name} โดย User ID {random_user_id}: {e}"
                    )

            saved_attractions_count += 1
        session.commit()
        print(
            f"บันทึกสถานที่และข้อมูลที่เกี่ยวข้องสำเร็จ! บันทึกไป {saved_attractions_count} รายการ, ข้ามไป {skipped_attractions_count} รายการ"
        )
    except Exception as e:
        session.rollback()
...
def get_and_display_data():
    ...
    try:
        ...
        for user in users:
            print(
                f"ID: {user.user_id}, Username: {user.username}, Email: {user.email}, Role: {user.role}"
            )  # แก้ไข: ใช้ user_id
        if not users:
        ...
        for cat in categories:
            print(
                f"ID: {cat.category_id}, Name: {cat.name}, Description: {cat.description}"
            )  # แก้ไข: ใช้ category_id
        if not categories:
            print("ไม่มีข้อมูลหมวดหมู่")
        ...
        for attr in attractions:
            ...
            print(
                f"ID: {attr.id}, ชื่อ: {attr.name}, หมวดหมู่: {category_name}, "
        ...
        if not attractions:
            print("ไม่มีข้อมูลสถานที่ท่องเที่ยว")

        print("\n--- ข้อมูลจากตาราง Image ---")
        images = session.query(Image).all()
        for img in images:
            print(
                f"ID: {img.id}, Attraction ID: {img.attraction_id}, URL: {img.image_url}, Caption: {img.caption}"
            )
        if not images:
            print("ไม่มีข้อมูลรูปภาพ")

        print("\n--- ข้อมูลจากตาราง Review ---")
        reviews = session.query(Review).all()
        for rev in reviews:
            print(
                f"ID: {rev.id}, Attraction ID: {rev.attraction_id}, User ID: {rev.user_id}, "
                f"Rating: {rev.rating}, Comment: {rev.comment[:50]}..., Created: {rev.created_at}"
            )
        if not reviews:
            print("ไม่มีข้อมูลรีวิว")

        print("\n--- ข้อมูลจากตาราง Favorite ---")
        favorites = session.query(Favorite).all()
        for fav in favorites:
            print(
                f"ID: {fav.id}, User ID: {fav.user_id}, Attraction ID: {fav.attraction_id}"
            )
        if not favorites:
            print("ไม่มีข้อมูลรายการโปรด")
        ...
        if not attraction_tags:
            print("ไม่มีข้อมูลแท็กของสถานที่")
...
if __name__ == "__main__":
    ...
    if (
        mock_attractions_data
        and all_user_ids
        and category_name_to_id
        and tag_name_to_id
    ):
        save_attractions_and_related_data(
            mock_attractions_data, all_user_ids, category_name_to_id, tag_name_to_id
        )
    else:
